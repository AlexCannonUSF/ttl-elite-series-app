...
public class PlayerIdentityService {
    ...
    public Player resolveOrCreatePlayer(String rawName) {
        ...
        Optional<PlayerAlias> alias = playerAliasRepository.findByNormalizedAlias(normalized);
        if (alias.isPresent()) {
            return alias.get().getPlayer();
        }

        Optional<Player> existing = playerRepository.findByNormalizedName(normalized);
        if (existing.isPresent()) {
            ensureAlias(existing.get(), rawName);
            return existing.get();
        }
    ...
    public Optional<Player> findCanonicalPlayer(String rawName) {
        ...
        Optional<PlayerAlias> alias = playerAliasRepository.findByNormalizedAlias(normalized);
        if (alias.isPresent()) {
            return Optional.of(alias.get().getPlayer());
        }
        Optional<Player> exact = playerRepository.findByNormalizedName(normalized);
        if (exact.isPresent()) {
            return exact;
        }
