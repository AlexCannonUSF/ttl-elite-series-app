...
export function PlayersRoute() {
  ...
  return <V3Shell title={selected ? selected.fullName : 'Players'} description={selected ? 'Complete form, ratings, matchup comparisons, and recent results with evidence depth always visible.' : 'Search the complete player database, inspect sample depth, and open any player for match-by-match intelligence.'} badges={<><Badge variant="accent"><Database className="mr-1 size-3" /> {players.length.toLocaleString()} players</Badge>{selected ? <Badge>{selectedStats?.matches ?? 0} matches</Badge> : null}</>}>
    {error ? <div className="mb-4 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-950">{error}</div> : null}
    {selected ? <PlayerProfile player={selected} statistics={selectedStats} matches={matches} matchup={matchup} allPlayers={players} compareId={compareId} watched={watchlist.some((item) => item.kind === 'PLAYER' && item.id === String(selected.id))} onToggleWatch={() => toggleWatchlist({ id: String(selected.id), kind: 'PLAYER', label: selected.fullName, detail: `${selectedStats?.matches ?? 0} recorded matches`, href: `/user/players/${selected.id}` })} /> : <PlayerExplorer rows={rows} query={query} setQuery={setQuery} sort={sort} setSort={setSort} />}
  </V3Shell>
}

function PlayerExplorer({ rows, query, setQuery, sort, setSort }: { rows: PlayerRow[]; query: string; setQuery: (value: string) => void; sort: 'MATCHES' | 'NAME' | 'WIN_PCT'; setSort: (value: 'MATCHES' | 'NAME' | 'WIN_PCT') => void }) {
  return <div className="grid gap-5 xl:grid-cols-[0.72fr_1.28fr]"><section className="user-hero rounded-[30px] border border-emerald-300/15 p-6 text-white"><Badge className="border-emerald-300/20 bg-emerald-300/10 text-emerald-100"><UserRound className="mr-1 size-3" /> Player intelligence</Badge><h2 className="mt-5 text-4xl font-semibold tracking-[-0.05em]">Know the player behind the price.</h2><p className="mt-4 max-w-xl text-sm leading-6 text-slate-300">Open form, schedule strength, H2H history, rating systems, and the model’s probability. Thin histories stay labeled instead of being presented as confidence.</p><div className="mt-7 grid grid-cols-3 gap-2"><HeroMini label="Database" value={rows.length.toLocaleString()} /><HeroMini label="With 50+" value={String(rows.filter((row) => (row.statistics?.matches ?? 0) >= 50).length)} /><HeroMini label="Search" value="Instant" /></div></section><Card><CardHeader><div className="grid gap-2 sm:grid-cols-[1fr_auto]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[var(--ink-muted)]" /><input aria-label="Search players" className="h-11 w-full rounded-xl border border-[var(--line)] bg-white/70 pl-9 pr-3 text-sm outline-none focus:border-emerald-400" onChange={(event) => setQuery(event.target.value)} placeholder="Search player name" value={query} /></label><select aria-label="Sort players" className="h-11 rounded-xl border border-[var(--line)] bg-white/70 px-3 text-xs font-semibold" onChange={(event) => setSort(event.target.value as typeof sort)} value={sort}><option value="MATCHES">Most matches</option><option value="WIN_PCT">Highest win rate</option><option value="NAME">Name</option></select></div></CardHeader><CardContent><div className="max-h-[680px] space-y-2 overflow-y-auto pr-1">{rows.slice(0, 300).map((row) => <Link className="grid grid-cols-[1fr_repeat(3,74px)_auto] items-center gap-3 rounded-2xl border border-[var(--line)] bg-white/60 p-3 transition hover:border-emerald-300 hover:bg-white" key={row.id} to={`/user/players/${row.id}`}><div className="min-w-0"><p className="truncate text-sm font-bold">{row.fullName}</p><p className="mt-0.5 text-[10px] text-[var(--ink-muted)]">Player #{row.id}</p></div><Mini label="Matches" value={integer(row.statistics?.matches)} /><Mini label="Record" value={`${row.statistics?.wins ?? 0}-${row.statistics?.losses ?? 0}`} /><Mini label="Win rate" value={pct(row.statistics?.winPct)} /><ArrowRight className="size-4 text-[var(--ink-muted)]" /></Link>)}</div></CardContent></Card></div>
}
