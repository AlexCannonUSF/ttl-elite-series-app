...
function EquityStrip({ points }: { points: number[] }) {
  const width = 680; const height = 210; const padX = 64; const padY = 30
  ...
  return <figure aria-labelledby="replay-equity-title">
    <div className="mb-2 flex flex-wrap items-end justify-between gap-2"><div><p id="replay-equity-title" className="text-xs font-bold text-[var(--ink-strong)]">Flat-$1 replay equity path</p><p className="mt-1 text-[10px] text-[var(--ink-muted)]">Running virtual bankroll after each deterministically replayed result.</p></div><span className="font-mono text-sm font-bold">Latest {money(points.at(-1) ?? 0)}</span></div>
    <svg aria-label="Virtual bankroll by replayed event sequence" className="w-full rounded-2xl border border-[var(--line)] bg-white/60" role="img" viewBox={`0 0 ${width} ${height}`}>
      <title>Replay bankroll curve</title><desc>Horizontal axis is replay event sequence. Vertical axis is virtual bankroll in dollars using one-dollar straight bets.</desc>
      {[0, .25, .5, .75, 1].map((ratio) => { const gridY = padY + ratio * (height - padY * 2); const value = max - ratio * range; return <g key={ratio}><line x1={padX} y1={gridY} x2={width - padX} y2={gridY} stroke="rgba(100,116,139,.15)" /><text x={padX - 8} y={gridY + 3} textAnchor="end" className="fill-slate-500 text-[9px]">{money(value)}</text></g> })}
      <path d={path} fill="none" stroke="rgb(37,99,235)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" />
      {points.map((point, index) => index === 0 || index === points.length - 1 || index % Math.max(1, Math.floor(points.length / 12)) === 0 ? <circle key={index} cx={x(index)} cy={y(point)} fill="rgb(37,99,235)" r="2.5"><title>{`Event ${index}: ${money(point)}`}</title></circle> : null)}
      {[0, .5, 1].map((ratio) => <text key={ratio} x={padX + ratio * (width - padX * 2)} y={height - 16} textAnchor={ratio === 0 ? 'start' : ratio === 1 ? 'end' : 'middle'} className="fill-slate-500 text-[9px]">{Math.round(ratio * Math.max(0, points.length - 1))}</text>)}
      <text x={width / 2} y={height - 3} textAnchor="middle" className="fill-slate-500 text-[9px] uppercase tracking-[0.14em]">Replay event sequence</text><text x="11" y={height / 2} transform={`rotate(-90 11 ${height / 2})`} textAnchor="middle" className="fill-slate-500 text-[9px] uppercase tracking-[0.14em]">Virtual bankroll · USD</text>
    </svg>
    <figcaption className="mt-2 text-[10px] text-[var(--ink-muted)]">This is a historical simulation based on frozen receipts, not a real-money result.</figcaption>
  </figure>
}
